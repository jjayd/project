...
batch_size = 16 # 16
encoder_size = 100 #100

decoder_size = 20 
#tool.check_doclength(title, sep=True)  # (Maximum) number of time steps in this batch
...
class seq2seq(object):
    ...
    def __init__(self, multi, hidden_size, num_layers, forward_only,
        ...
        # models
        if multi:
          #  cell = tf.nn.bidirectional_dynamic_rnn(output_projection)
            single_cell_fw = tf.nn.rnn_cell.GRUCell(num_units=hidden_size)
            cell_fw = tf.nn.rnn_cell.MultiRNNCell([single_cell_fw] * num_layers)

            single_cell_bw = tf.nn.rnn_cell.GRUCell(num_units=hidden_size)
            cell_bw = tf.nn.rnn_cell.MultiRNNCell([single_cell_bw] * num_layers)
...
while current_step < 201:
    ...
    if current_step % steps_per_checkpoint == 0:
        ...
        maximum = max(predict)
        f.write(" iteration %d : "%current_step)
        for i in predict:
            f.write('<font style="background: rgba(251, 183, 197, %f)">%s</font>\n' % (i/maximum,ix_to_word[i]))
        f.write("<br></br>")
            
       # for i in predict:
       #     print(i)
       #     print(ix_to_word[i][0])
        
        predict = ' '.join(ix_to_word[ix] for ix in predict)
        real = [word[0] for word in targets]
        real = ' '.join(ix_to_word[ix][0] for ix in real)
        print('step : %s , loss : %s \n Prediction : %s \n Answer : %s \n\n' %
              (current_step, loss, predict, title[start]))
